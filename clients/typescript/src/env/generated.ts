...
export function loadFrom(lookup: (key: string) => string | undefined): ClientEnvValues {
  return {
    api_base: nonEmpty(lookup("FLAGS_2_ENV_API_BASE")),
    token: nonEmpty(lookup("FLAGS_2_ENV_TOKEN")),
  };
}
...
export function loadEnvMap(
  ...
  out["FLAGS_2_ENV_API_BASE"] = requireEnv("FLAGS_2_ENV_API_BASE", "string", ["http://127.0.0.1:8080", "https://api.flags2env.example"], api_base);
  const token = pick(["FLAGS_2_ENV_TOKEN"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, undefined);
  if (token !== undefined) out["FLAGS_2_ENV_TOKEN"] = token;
  return out;
}
